...
async function main() {
  ...
  // For most sessions, create at least one TaskInUse with isSessionTask: false and a valid taskDefinitionId
  // For a minority, create only a free practice TaskInUse (isSessionTask: true, no taskDefinitionId)

  // Helper: map session index to relevant taskDefinition and tags
  ...
  // Let's say sessions[5] and sessions[12] are free practice
  await prisma.taskInUse.create({
    data: {
      duration: 500,
